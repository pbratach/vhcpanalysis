...
TOOL_VERSION="1.2.0.2025-Sep-05"
...
# Parse the region name in the INPUT_FILE name, remove the prefix up to the first dot, then remove the suffix up to the first _
REGION_NAME=$(basename $(echo ${INPUT_FILE}) | awk -F '[._]' '{print $2}')
if [[ "$VERBOSE" == "1" ]]; then
	log "Region name is ${REGION_NAME}"
fi
...
if [[ ! -d ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME} ]]; then
    mkdir -p ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME}
fi
...
# Unpack log tar file
#
log "Unpacking tar file ${INPUT_FILE} into ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME}" 
tar xf ${INPUT_FILE} -C ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME}
log " "

# Change directory to the unpacked log tar file
cd ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME}
...
cd ${CURRENT_DIR}/${IF_TIMESTAMP}/${REGION_NAME}/cluster
if [[ "$VERBOSE" == "1" ]]; then
	pwd
	ls -l
fi
