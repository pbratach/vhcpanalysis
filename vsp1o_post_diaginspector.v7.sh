...
# Version 7
#  Created standard function for displaying log files
######################################################
...
echo " "
echo "Script version 1.7 (04/03/2026)"
...
process_log_output() {
    ...
    # If no lines found, exit silently
    if [[ -z "$filtered" ]]; then
        return
    fi

    # Sort by first two columns (date + time)
    local sorted
    if [[ "$timecols" -eq 2 ]]; then
        sorted=$(printf "%s\n" "$filtered" | sort -k1,2)
    else
        sorted=$(printf "%s\n" "$filtered" | sort -V -k1,1)
    fi
    ...
    if (( count < 10 )); then
        ...
        echo -e "\033[0;31mThere are $count occurrences of this error, only displaying the first 2 and the 2 most recent errors in the logs\033[0m"
        echo -e "\033[0;31mIssue the command \033[0m${findcmd} \033[0;31mto see all the errors\033[0m"
...
if [ $(echo "$output" | wc -l) -gt 1 ]; then
  findcmd="find . -name \"s3-g*s3-g*.log*\" -exec grep -B 1 \"The difference between the request time and the server's time is too large\" {} \; | grep \"S3ServletAuthenticator\" | awk '{print $2}' | sort"
  process_log_output "$findcmd" "$output" "Too big clock skew is detected" 1
  echo -e "\033[0;31m##########################################\033[0m"
fi
...
if [ $(echo "$output" | wc -l) -gt 1 ]; then
  findcmd="find . -name \"*yb*server.log*\" -exec grep \"Too big clock skew is detected\" {} \;"
  process_log_output "$findcmd" "$output" "Too big clock skew is detected" 2
  echo -e "\033[0;31m##########################################\033[0m"
fi
