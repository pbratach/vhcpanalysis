...
END {
    printf "%-6s %12s %16s %12s %12s %10s\n", "METHOD", "COUNT", "TOTAL_SIZE", "MIN_TIME", "MAX_TIME", "AVG_TIME"
    for (k in count) {
    printf "%-6s %'12d %'16d %'12d %'12d %10.2f\n", op, count[op], sizes[op], min[op], max[op], avg
    }
}
AWK
...
| sort -k 2
{
  # Get method token and strip leading/trailing quotes
  op = $2
  gsub(/^"|"$/, "", op)

  # Split full line on spaces
  n = split($0, f, " ")

  # Time is the 12th field
  time = f[12]

  # Size logic by method
  if (op == "PUT")              size = f[10]
...
END {
  # Header
  printf "%-6s %12s %16s %12s %12s %10s\n", "S3 CALL", "COUNT", "TOTAL_SIZE", "MIN_TIME", "MAX_TIME", "AVG_TIME"

  # Rows
  for (op in count) {
    avg = times[op] / count[op]
    printf "%-6s %'12d %'24d %'12d %'12d %10.2f\n",
           op, count[op], sizes[op], min[op], max[op], avg
  }
}
AWK
