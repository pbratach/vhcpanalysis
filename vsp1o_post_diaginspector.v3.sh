...
echo " "
echo "Checking for occurrences of WARNING : KI-1 : Time sync issue between VSP Object nodes and S3 client/application error"
grep -B 1 "The difference between the request time and the server's time is too large" */s3-g*s3-g*.log* | grep "S3ServletAuthenticator" | awk '{print $2}' | sort
