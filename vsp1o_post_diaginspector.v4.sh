...
#
echo "Version 4, updated 12/31/2025"
...
echo " "
echo -e "\033[0;32mCounting the total number of S3 calls in istio s3 logs\033[0m"
...
num=$(wc -l  all-s3-gateway-istio-proxy.txt | awk '{print $1}')
fnum=$(echo $num | sed ':a;s/\B[0-9]\{3\}\>/,&/;ta')
echo -e "\033[0;32mTotal number of S3 calls in s3-gateway istio-proxy logs:\033[0m $fnum"
# Count the number of downstream_remote_disconnect errors in istio s3 logs
echo -e "\033[0;32mTotal number of downstream_remote_disconnect errors in S3 calls in s3-gateway istio-proxy logs:\033[0m `grep downstream_remote_disconnect all-s3-gateway-istio-proxy.txt | wc -l`"
# Remove the downstream_remote_disconnect errors in istio s3 logs
echo -e "\033[0;32mProcessing downstream_remote_disconnect errors in S3 calls in s3-gateway istio-proxy logs\033[0m"
...
# Count the number of S3 calls ignoring downstream_remote_disconnect errors in istio s3 logs
echo -e "\033[0;32mTotal number of S3 calls excluding downstream_remote_disconnect errors in s3-gateway istio-proxy logs:\033[0m `wc -l all-s3-gateway-istio-proxy.txt-no-downstream_remote_disconnect.txt | awk '{print $1}'`"
...
if [ $(echo "$output" | wc -l) -gt 1 ]; then
  echo " "
  echo "Found occurrences of WARNING : KI-1 : Time sync issue between VSP Object nodes and S3 client/application error"
  echo "$output"
fi
